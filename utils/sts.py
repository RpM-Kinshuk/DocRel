...
def save_scores(
    ...
    top_results = current_df.sort_values(
        by="Similarity", ascending=False if sim_measure == "cosine" else True
    ).head(int(top_n))
    return top_results
...
def similarity_scores(query, top_n, model, sim_measure):
    ...
    cos_sim_scores, euclid_sim_scores = calculate_similarity_scores(
        abstract_embeddings, query_embedding
    )
    keyword_overlaps = calculate_overlap(abstracts, query)

    top_results = save_scores(
        dataset,
        cos_sim_scores,
        euclid_sim_scores,
        keyword_overlaps,
